...
```{r}
lollipop_chart(diamonds, cut)
ggcharts_set_theme("theme_hermit")
lollipop_chart(diamonds, cut)
```

The advantage of using `ggcharts_set_theme()` over adding the theme to a `ggcharts` plot is two-fold. First, as already mentioned the theme arguments are set automatically. In addition, the default color used for the geom is changed as can be seen above.
